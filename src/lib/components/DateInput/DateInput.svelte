<script lang="ts">
	...
	const getSegmentValue = (part: string, value: string) => {
		if (part === 'day') {
			return selectedValue
				? value.padStart(2, '0')
				: today(getLocalTimeZone()).day.toLocaleString().padStart(2, '0');
		}
		if (part === 'month') {
			return selectedValue
				? value.padStart(2, '0')
				: today(getLocalTimeZone()).month.toLocaleString().padStart(2, '0');
		}
	...
	<div class="flex w-full flex-col gap-1.5">
		...
		>
			{#snippet children({ segments })}
				<DatePicker.Trigger
					class="flex w-full cursor-pointer transition-all disabled:cursor-not-allowed"
				>
				...
				{#snippet children({ months, weekdays })}
                  ...
                  `}
								>
									{#snippet children({ selectedYearItem, yearItems })}
										{#each yearItems as { value, label }, i (value)}
											<option
												{value}
												selected={selectedYearItem?.value === value}
								...
								<DatePicker.GridHead>
									<DatePicker.GridRow class="flex w-full">
										{#each weekdays as day, index (day + index)}
											<DatePicker.HeadCell
												class="flex h-12 w-full items-center justify-center rounded-md font-normal!"
											>
					...
					<div class="flex justify-between px-3 pt-2 pb-3">
						<Button variant="ghost" shape="round" size="standard" onclick={handleClear}
							>Clear</Button
						>
						<div class="flex">
							<Button variant="ghost" shape="round" size="standard" onclick={handleCancel}
								>Cancel</Button
							>
